import { options as opts } from "./option-manager"

export const randomInt = function (min, max) {
	return Math.round(min + (Math.random() * (max - min)));
};

export const stripCharsFromString = function (string, chars) {
	return string.split('').filter(char => chars.indexOf(char) === -1);
};

export let int = randomInt;

export let greyColor = function (min, max) {
	min = min || 1;
	max = max || 9;
...
export let captchaText = function (options) {
...
export const mathExprPlus = function(leftNumber, rightNumber){
	const text = (leftNumber + rightNumber).toString();
	const equation = leftNumber + '+' + rightNumber;
	return {text, equation}
}

export const mathExprMinus = function(leftNumber, rightNumber){
	const text = (leftNumber - rightNumber).toString();
	const equation = leftNumber + '-' + rightNumber;
	return {text, equation}
}
 ...
 */
export let mathExpr = function (min, max, operator) {
	min = min || 1;
	max = max || 9;
	operator = operator || '+';
...
export let color = function (bgColor) {
	// Random 24 colors
	// or based on step
